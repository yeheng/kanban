...
impl ResourcesService {
    ...
    pub async fn create(pool: &SqlitePool, name: &str, email: Option<&str>) -> Result<i64, AppError> {
        let id = ResourcesRepo::create(pool, name, email).await?;
    ...
    #[tracing::instrument(skip(pool), fields(resource_id = id))]
    pub async fn soft_delete(pool: &SqlitePool, id: i64) -> Result<(), AppError> {
        ResourcesRepo::soft_delete(pool, id).await?;
        tracing::info!(resource_id = id, "deleted resource");
        Ok(())
    }

    #[tracing::instrument(skip(pool), fields(resource_id = id, name = %name))]
    pub async fn update(
        ...
        if let (Some(s), Some(e)) = (available_from, available_to) {
            if e < s {
                tracing::warn!(resource_id = id, start = %s, end = %e, "invalid resource availability window");
                return Err(domain::DomainError::InvalidDateWindow.into());
            }
        }
        if let Some(capacity) = daily_capacity_pd {
            if !capacity.is_finite() || capacity <= 0.0 {
                tracing::warn!(resource_id = id, daily_capacity_pd = capacity, "invalid daily capacity");
        ...
        if let Some(rate) = daily_rate_pd {
            if !rate.is_finite() || rate < 0.0 {
                tracing::warn!(resource_id = id, daily_rate_pd = rate, "invalid daily rate");
                return Err(domain::DomainError::InvalidValue { field: "daily_rate_pd", value: rate }.into());
            }
        }
        ResourcesRepo::update(pool, id, name, email, available_from, available_to,
    ...
    #[tracing::instrument(skip(pool, skills), fields(resource_id = id, skill_count = skills.len()))]
    pub async fn set_skills(
        pool: &SqlitePool, id: i64, skills: &[(i64, i64)],
    ) -> Result<(), AppError> {
        for &(sid, prof) in skills {
            if !(1..=5).contains(&prof) {
                tracing::warn!(resource_id = id, skill_id = sid, proficiency = prof, "invalid proficiency");
            ...
            if exists.is_none() {
                tracing::warn!(resource_id = id, skill_id = sid, "skill not found");
                return Err(domain::DomainError::NotFound(format!("skill {}", sid)).into());
            }
        }
        ResourcesRepo::set_skills(pool, id, skills).await?;
        tracing::info!(resource_id = id, skill_count = skills.len(), "set resource skills");
        Ok(())
    }
    ...
    #[tracing::instrument(skip(pool, tag_ids), fields(resource_id = id, tag_count = tag_ids.len()))]
    pub async fn set_tags(pool: &SqlitePool, id: i64, tag_ids: &[i64]) -> Result<(), AppError> {
        for &tid in tag_ids {
            ...
            if exists.is_none() {
                tracing::warn!(resource_id = id, tag_id = tid, "tag not found");
                return Err(domain::DomainError::NotFound(format!("tag {}", tid)).into());
            }
        }
        ResourcesRepo::set_tags(pool, id, tag_ids).await?;
        tracing::info!(resource_id = id, tag_count = tag_ids.len(), "set resource tags");
        Ok(())
    }
}
