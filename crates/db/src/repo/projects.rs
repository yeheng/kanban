...
impl ProjectsRepo {
    #[tracing::instrument(skip_all, level = "debug", fields(name, priority))]
    pub async fn create(
    ...
    pub async fn update(
        ...
        let n = sqlx::query(
            "UPDATE projects SET name=?, description=?, start_date=?, end_date=?, \
                    priority=?, budget_pd=?, updated_at=strftime('%Y-%m-%dT%H:%M:%SZ','now') \
             WHERE id=? AND deleted_at IS NULL")
            .bind(name).bind(description).bind(start).bind(end).bind(priority).bind(budget_pd).bind(id)
